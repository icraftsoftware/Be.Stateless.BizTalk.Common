...
namespace Be.Stateless.BizTalk.Streaming
{
	...
	/// </para>
	/// <para>
	/// In detailed terms, regardless of whether this stream's client will assume the end-of-stream has been reached as soon as a
	/// <see cref="Stream.Read"/> operation returns less bytes than the number of bytes requested, or whether it will only assume
	/// the end-of-stream has been reached once the <see cref="Stream.Read"/> operation returns zero (0) &#8212; which is, by the
	/// way, the officially documented way of being notified the end-of-stream has been reached &#8212; an <see
	/// cref="EventingReadStream"/> instance will always fire the <see cref="IProvideReadStreamEvents.AfterLastReadEvent"/> event
	/// as soon as the inner stream's end has been reached.
	/// </para>
	/// </remarks>
	public class EventingReadStream : Microsoft.BizTalk.Streaming.EventingReadStream
	{
		...
		public override bool CanSeek
		{
			get
			{
				...
				{
					ThrowIfDisposed();
					// do not mess around with current position and corrupt fired events while stream is still being read for the 1st time
		...
		/// known accurately once the whole stream has been exhausted. While being read, <see cref="Length"/> will therefore throw
		/// a <see cref="NotSupportedException"/> exception.
		/// </remarks>
