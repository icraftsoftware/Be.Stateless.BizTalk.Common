...
namespace Be.Stateless.BizTalk.XPath
{
	...
	public class PropertyExtractorCollectionFixture
	{
		...
		{
			var xml = $"<san:Properties precedence='schemaOnly' xmlns:s0='urn' xmlns:san='{SchemaAnnotations.NAMESPACE}'>"
		...
		public void ReadXmlThrowsWhenRootElementMissesNamespace()
		{
			...
			using (var reader = XmlReader.Create(new StringReader(xml)))
			{
				...
				Action(() => sut.ReadXml(reader))
					.Should().Throw<ConfigurationErrorsException>()
					.WithInnerException<XmlException>()
					.WithMessage($"Element 'Properties' with namespace name '{SchemaAnnotations.NAMESPACE}' was not found. Line 1, position 2.");
			}
		}
